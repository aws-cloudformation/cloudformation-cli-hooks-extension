...
class DescribeHookExtension(ExtensionPlugin):
    ...
    def _describe_hook(self, args: Namespace) -> None:
        """
        Main method for the describe-hook command. Displays the information for the current hook project,
        ...
        hook_data = self._get_hook_data(type_name)
        ...
        compiled_target_handlers_string = self._build_target_handlers_string(versioned_hook_data, hook_configuration_data)
    ...
    def setup_parser(self, parser):
        parser.set_defaults(command=self._describe_hook)
